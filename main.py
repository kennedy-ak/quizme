...
def generate_questions(pdf_text, num_questions, question_types, difficulty):
    ...
    llm = ChatGroq(
        ...
        model_name="gemma2-9b-it" # You can change this to other Groq models as needed
    )
    ...
    max_tokens = 16,000  # This is a rough estimate; adjust based on the model's context window
    ...
    for i, (question, answer) in enumerate(zip(st.session_state.questions, st.session_state.answers)):
        with st.container():
            ...
            if question["type"] == "multiple_choice":
                ...
                st.markdown(f"""
                <p>Your answer: <span class="{'correct-answer' if is_correct else 'incorrect-answer'}">{user_answer or 'No answer'}</span></p>
                ...
                st.markdown(f"""
                <p>Your answer: <span class="{'correct-answer' if is_correct else 'incorrect-answer'}">{user_answer}</span></p>
                <p>Correct answer: <span class="correct-answer">{correct}</span></p>
                """, unsafe_allow_html=True)
